...
![The menu bar](assets/02-nav--header.png)
...
| **"मास्टर टेम्पलेट"** (Master templates) | *(Admin)* The poster template library |
...
![The tasks panel when nothing is running](assets/02-nav--tasks-empty.png)

While an article is being generated, the same panel shows the run with its live step:

![The tasks panel with an article run in progress](assets/02-nav--tasks-article-running.png)

Inside the panel:

* Each row shows the work's title, a status dot, and the **live step** it is on (for example **"पोस्टरचे चित्र तयार करत आहोत…"** — *preparing the poster image*). The dot pulses while the work is running.
...
One more rule to know: the platform runs **one article-type job and one Twitter job at a time**. While an article is being generated, the article cards on the create form are disabled (Twitter stays available, and vice versa). The form explains this with a message such as **"एक लेख सध्या तयार होत आहे. तो पूर्ण झाल्यावर नवीन सुरू करता येईल."** (*An article is currently being created. A new one can be started once it finishes.*)
...
![The mobile menu](assets/02-nav--mobile-menu.png)
