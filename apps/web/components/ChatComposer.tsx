...
export function ChatComposer({
  ...
  const trayItems: TrayAttachment[] = attachments.map((attachment) => {
    ...
    return {
      ...
      // left here to trim.
      //
      // TRIGGER DISABLED ON THIS BRANCH. Trimming is reachable from the DLO and transcribe
      // composers of the new design only, and this branch runs the production UI, where those
      // composers do not exist — so leaving the gesture on /chat alone would make the feature
      // look half-present. Everything else about the trim stays in place and working: the
      // dialog below, useChatAttachments' `trim` state, the `audioTrims` field on both create
      // routes, and the ffmpeg cut itself. Uncomment these two lines to bring it back.
      //
