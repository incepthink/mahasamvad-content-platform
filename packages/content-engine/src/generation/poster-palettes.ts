// The curated palette library used by landscape article posters, plus its seeded picker.
// Fresh social posters now use the continuous OKLCH generator in social-colour-plan.ts; this file
// remains the shared palette shape and resolves both legacy fixed ids and procedural social ids.
//
// This fixed catalogue remains intentionally stable because its ids are persisted and article
// prompts depend on the exact values. The social generator uses the same PosterPalette contract,
// so persistence, labels and measurement history continue to work across both approaches.

import { pathToFileURL } from 'node:url';
import { socialPaletteById } from './social-colour-plan.js';
...
export type PaletteFamily =
  'cool' | 'teal' | 'green' | 'purple' | 'neutral' | 'warm';
...
// The colour roles a poster is built from. Exact values, because an image model obeys `#1F3A5F`
...
export type PaletteHex = Readonly<{
  ...
  ink: string;
  // Text sitting on the dominant colour. Procedural social palettes always provide it; legacy
  // article palettes predate the role and continue to rely on the image model's contrast choice.
  textOnPanel?: string | undefined;
...
export const POSTER_PALETTES: readonly PosterPalette[] = [
  // --- cool: blue / indigo / slate, on a cool porcelain ground ------------------------------
  {
    ...
    palette:
      'a deep royal indigo panel on a cool porcelain ground, with a warm amber accent',
    background:
      'a cool pale-porcelain background with a deep-indigo colour block',
    accent: 'warm amber',
  },
  {
    id: 'slate_steel',
    ...
    palette:
      'a mid slate-blue panel on a misty blue-grey ground, with a burnt-coral accent',
  ...
  },
  {
    ...
    palette:
      'a bright cobalt panel on an icy pale-blue ground, with a golden-yellow accent',
  ...
  // --- teal: teal / cyan / aqua, on a pale aqua-grey ground ---------------------------------
  {
    ...
    palette:
      'a deep teal panel on a pale aqua-grey ground, with a coral accent',
  ...
  },
  {
    ...
    palette:
      'a sea-green panel on a shell-white ground tinted green, with a soft-gold accent',
    background:
      'a green-tinted shell-white background with a sea-green colour block',
    accent: 'soft gold',
  },
  {
    id: 'petrol_cyan',
    ...
    palette:
      'a dark petrol-blue panel on a cyan-tinted mist ground, with a bright cyan accent',
    background:
      'a cyan-tinted pale background with a dark petrol-blue colour block',
    accent: 'bright cyan',
  },
  ...
  // --- green: forest / olive / emerald, on a mint-grey ground -------------------------------
  {
    ...
    palette:
      'a forest-green panel on a mint-grey ground, with a brass-gold accent',
  ...
  },
  {
    ...
    palette:
      'a muted sage panel on a pale moss ground, with a soft rust accent',
  ...
  },
  {
    ...
    palette:
      'a vivid emerald panel on a pale lime-tinted ground, with a deep-navy accent',
    background:
      'a pale lime-tinted background with a vivid emerald colour block',
    accent: 'deep navy',
  },
  ...
  // --- purple: plum / violet / aubergine, on a lilac-grey ground -----------------------------
  {
    ...
    palette:
      'a deep plum panel on a lilac-grey ground, with a champagne-gold accent',
  ...
  },
  {
    ...
    palette:
      'a dark aubergine panel on a dusty-rose ground, with a muted-clay accent',
  ...
  // --- neutral: charcoal / graphite / stone, one vivid accent doing all the work -------------
  {
    ...
    palette:
      'a charcoal panel on a light neutral-grey ground, with a vivid emerald accent',
  ...
  },
  {
    ...
    palette:
      'a graphite panel on a bone-white ground, with a signal-red accent',
  ...
  },
  {
    ...
    palette:
      'a near-black navy panel on a cool stone-grey ground, with a pale-blue accent',
    background:
      'a cool stone-grey background with a near-black navy colour block',
  ...
  // --- warm: the DGIPR house look and its neighbours. THREE of eighteen, on purpose. ----------
  {
    ...
    palette:
      'the DGIPR brand saffron-orange panel on a warm cream ground, with a deep-maroon accent',
    background:
      'a warm cream / paper background with saffron-orange colour bands',
    accent: 'deep maroon',
  },
  {
    id: 'deep_burgundy',
    ...
    palette:
      'a deep burgundy panel on an off-white warm ground, with an antique-gold accent',
  ...
  },
  {
    ...
    palette:
      'a terracotta panel on a warm sand ground, with a deep-teal accent',
...
export function paletteById(
  ...
  if (!id) return null;
  return POSTER_PALETTES.find((p) => p.id === id) ?? socialPaletteById(id);
}
...
if (
  ...
  for (let i = 1; i <= FAMILY_RING; i += 1) {
    for (let j = i; j < picks.length; j += 1) {
      ...
      if (a.family === b.family) {
        failures.push(
          `family ${a.family} repeated at runs ${j - i} and ${j} (gap ${i})`,
        );
      }
    }
  }
  ...
  if (warm > Math.ceil(RUNS / 4)) {
    failures.push(
      `warm family used ${warm}/${RUNS} times — should be near ${RUNS / 6}`,
    );
  }
  ...
  for (const p of POSTER_PALETTES) {
    ...
    if (p.family !== 'warm' && red - blue > 8) {
      failures.push(
        `${p.id} (${p.family}) has a warm ground ${p.hex.ground} — should be tinted to its family`,
      );
    }
  }
