...
const ANALYSIS_PROMPT = [
  ...
  '',
  '"contentSummary" (string): 1-2 short sentences naming what this specific poster is ABOUT — its scheme, subject, announcement or event — so a person can recognise it in a library at a glance. UNLIKE layoutSummary, here you SHOULD read the placeholder text and name the actual topic (e.g. the scheme name, the department, the announcement). ALWAYS write it in MARATHI (Devanagari), whatever language the poster itself is in — a Hindi, Gujarati or English master must still be summarised in Marathi, because these summaries are compared against Marathi notes when a reference is picked. Do NOT transcribe the full text; do NOT describe layout or colours here; ignore the emblem/title band/footer.',
].join('\n');
